...
def build_waveshare_can_packet(protocol: ProtocolConfig, frame_id: int, payload: bytes) -> bytes:
    ...
    is_fixed = (
        protocol.waveshare_fixed_20_bytes
        or protocol.parser_type == "waveshare_can_20_bytes"
    )
    if is_fixed:
        ...
        packet[1] = 0x55
        packet[2] = 0x01
        packet[3] = 0x02 if is_extended else 0x01
        packet[4] = 0x01
        packet[5:9] = frame_id.to_bytes(4, byteorder='little')
        packet[9] = dlc
        packet[10:10+dlc] = payload
        packet[19] = (sum(packet[:19]) + 1) & 0xFF
        return bytes(packet)
