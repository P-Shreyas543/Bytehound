...
def write_workbook_from_tables(tables: dict[str, pd.DataFrame], target_path: str | Path) -> Path:
    ...
    with pd.ExcelWriter(target, engine="openpyxl") as writer:
        ...
        readme.append(["   - In Protocol sheet: Set parser_type = 'waveshare_can'. (Header, footer, and CRC auto-default)."])
        readme.append(["   - In Variables sheet: Put CAN IDs in id_or_address (e.g. 0x0123 for standard ID or 0x18FF50E5 for extended ID)."])
        readme.append([])
